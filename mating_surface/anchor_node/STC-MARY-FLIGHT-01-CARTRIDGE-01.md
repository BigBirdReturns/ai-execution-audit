...
The verifier walks the cartridge tree without following links. A symlink or junction at the root, in any expected directory, or at any member path is refused before classification or reading, and traversal never descends through the linked entry. The Windows hostile witnesses construct actual directory junctions through native `mklink /J`, then exercise cartridge roots, linked ancestors, linked internal members, build destinations, verdict outputs, and projection outputs against those junctions. Every authoritative member, including `MANIFEST.json`, is limited to 1 MiB and read through a bounded `maximum + 1` operation after a size precondition. The embedded verifier retains its stricter exact-size, exact-digest, measured-byte, and stored-member binding.
