...
All cartridge verification, projection, verdict-output, and build-output coordinates require standard-library Python 3.12 or newer. Each entrypoint rejects a parent-directory segment before normalization and inspects every lexical absolute component before resolution. A symlink or Windows junction in the final component or any existing ancestor terminates structured refusal. A parent-directory segment remains refused even when normalization would appear to return to an ordinary sibling outside the linked component. Build, verdict, and projection outputs therefore cannot be redirected through a linked ancestor.

The verifier walks the cartridge tree without following links. A symlink or junction at the root, in any expected directory, or at any member path is refused before classification or reading, and traversal never descends through the linked entry. The Windows hostile witnesses use the same native `mklink /J` helper for directly linked roots, linked ancestors, internal members, build destinations, verdict outputs, and projection outputs; they contain no symbolic-link fallback or `skipTest` escape. Every authoritative member, including `MANIFEST.json`, is limited to 1 MiB and read through a bounded `maximum + 1` operation after a size precondition. The embedded verifier retains its stricter exact-size, exact-digest, measured-byte, and stored-member binding.
