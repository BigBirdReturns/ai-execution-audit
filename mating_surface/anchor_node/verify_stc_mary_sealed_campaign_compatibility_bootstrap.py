"""External bootstrap for the sealed-campaign compatibility verifier.

The standalone verifier reports ``bootstrapAuthenticated: false`` when it is called
...
1. measured the frozen verifier bytes on disk;
2. copied those measured bytes into a foreign temporary directory outside the
   repository and outside every campaign coordinate;
3. executed the measured copy in an isolated interpreter (``-I -S``) whose working
   directory is that foreign temporary directory;
4. validated the direct receipt, including that the verifier bound the stored repair
   source member to the exact bytes that executed.

The verifier is never imported, and the on-disk file is never executed in place.
"""
...
BOOTSTRAP_SCHEMA = "stc-mary/sealed-campaign-compatibility-bootstrap-verdict/1"
VERIFIER_FILENAME = "verify_stc_mary_sealed_campaign_compatibility.py"
AUTHORITY = "none"
MINIMUM_PYTHON = (3, 12)
...
namespace = {
    "__name__": "__main__",
    "__file__": "<measured-stc-mary-sealed-campaign-compatibility-verifier>",
    "_STC_MARY_BOOTSTRAP_MEASURED_VERIFIER_BYTES": source,
}
sys.argv = ["<measured-verifier>"] + sys.argv[1:]
exec(
    compile(source, "<measured-stc-mary-sealed-campaign-compatibility-verifier>", "exec"),
    namespace,
    namespace,
)
"""
...
def measure_verifier(anchor: Path) -> tuple[Path, bytes, str]:
    ...
    if coordinate_component_is_link(verifier, code="VERIFIER_MISSING", label="verifier") or not verifier.is_file():
        fail("VERIFIER_MISSING", "the admitted verifier is missing, linked, or not a regular file")
    size = verifier.stat().st_size
    if size > MAX_VERIFIER_BYTES:
        fail("VERIFIER_SIZE_INVALID", "the admitted verifier exceeds the bounded read allocation")
    with verifier.open("rb") as handle:
        data = handle.read(MAX_VERIFIER_BYTES + 1)
    if len(data) != size:
        fail("VERIFIER_SIZE_INVALID", "the admitted verifier changed during the bounded read")
    return verifier, data, sha256_bytes(data)


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Measure and isolate the STC MARY sealed-campaign compatibility verifier, then execute the measured bytes"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        verifier, verifier_bytes, observed = measure_verifier(anchor)

        output = None
        if args.out is not None:
            output = validate_lexical_coordinate(args.out, label="verdict output coordinate", code="VERDICT_PATH_INVALID")
            for forbidden, label in (
                (args.workstation, "workstation"),
                (args.repair_source_root, "repair source root"),
            ):
        ...
        scratch = tempfile.mkdtemp(prefix="stc-mary-sealed-compat-")
        foreign = Path(scratch).resolve()
        # The isolation directory must be foreign to every surface this run measures or
        # reads: the repair source root, the frozen workstation, and the predecessor
        # checkout. It is deliberately NOT compared against the bootstrap's own parent
        # tree, which may legitimately be a staging directory during qualification.
        for measured, label in (
            (args.repair_source_root, "repair source root"),
            (args.workstation, "workstation"),
            (args.conductor_checkout, "predecessor conductor checkout"),
        ):
        ...
        if completed.returncode != 0 or receipt.get("status") != "PASS":
            fail(str(receipt.get("code", "VERIFIER_REFUSED")), str(receipt.get("message", "the measured verifier refused")))
        ...
        if receipt.get("measuredVerifierSha256") != observed:
            fail(
                ...
                "the measured verifier did not bind the stored repair source member to the executed bytes",
            )
        if receipt.get("packageMutated") is not False or receipt.get("packetStagesReplayed") is not False:
            fail("VERDICT_MUTATION_STATE_INVALID", "the measured verifier reported mutation of a frozen surface")
