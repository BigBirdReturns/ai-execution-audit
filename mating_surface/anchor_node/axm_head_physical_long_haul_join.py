...
def write_carrier(
    ...
    out: Path,
    proof_root: dict[str, Any] | None = None,
) -> dict[str, Any]:
    global _ACTIVE_PRIVATE_PROOF_ROOT
    previous = _ACTIVE_PRIVATE_PROOF_ROOT
    _ACTIVE_PRIVATE_PROOF_ROOT = proof_root
    try:
        return _legacy_write_carrier(profile, catalog, input_value, out)
    finally:
        _ACTIVE_PRIVATE_PROOF_ROOT = previous

def create_private_proof_root_file(campaign: str, out: Path) -> dict[str, Any]:
    ensure_external_output(out)
    root = create_private_proof_root(campaign)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_bytes(canonical_json_bytes(root))
    try:
        out.chmod(0o600)
    except OSError:
        pass
    return {
        "schema": "axm-head/private-proof-root-creation@1",
        "status": "PASS",
        "campaignId": root["campaignId"],
        "proofRootId": root["proofRootId"],
        "keySha256": root["keySha256"],
        "privateRootWritten": True,
        "privateRootEmbedded": False,
        "physicalExecutionStartedByJoin": False,
        "workersLaunched": 0,
        "listenersCreated": 0,
        "authority": "none",
...
def authenticate_private_input_file(input_path: Path, proof_root_path: Path, out: Path) -> dict[str, Any]:
    ensure_external_output(out)
    root = load_private_proof_root(proof_root_path)
    value = authenticate_private_input(read_json(input_path), root)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_bytes(canonical_json_bytes(value))
    return {
        "schema": "axm-head/private-input-authentication@1",
        "status": "PASS",
        "campaignId": value["campaignId"],
        "proofRootId": root["proofRootId"],
        "graphSha256": value["privateDisposition"]["proof"]["graphSha256"],
        "privateInputWritten": True,
        "privateRootEmbedded": False,
        "physicalExecutionStartedByJoin": False,
        "workersLaunched": 0,
        "listenersCreated": 0,
...
def bootstrap_verify(carrier: Path, out: Path | None, proof_root: dict[str, Any] | None = None) -> dict[str, Any]:
    carrier = carrier.resolve()
    if out is not None:
        repository = REPOSITORY_ROOT.resolve()
        candidate = out.resolve(strict=False)
        if candidate == repository or repository in candidate.parents:
            fail("REPOSITORY_OUTPUT_REFUSED", "verdict output")
    ...
    if profile_bytes != canonical_json_bytes(profile_value) or catalog_bytes != canonical_json_bytes(catalog_value):
        fail("BOOTSTRAP_SOURCE_AUTHENTICATION_FAILED", "noncanonical recovery source")
    if (
        sha256_bytes(canonical_json_bytes(profile_value)) != PROFILE_CANONICAL_SHA256
        or sha256_bytes(canonical_json_bytes(catalog_value)) != FIXTURE_CATALOG_CANONICAL_SHA256
        or sha256_bytes(verifier_bytes) != STANDALONE_VERIFIER_SHA256
    ):
        fail("BOOTSTRAP_SOURCE_AUTHENTICATION_FAILED", "recovery identities")
    if proof_root is not None:
        _root_body(proof_root)
    with tempfile.TemporaryDirectory(prefix="axm-head-join-v2-snapshot-") as tmp:
        ...
        if proof_root is not None:
            root_path = temporary / "private-proof-root.json"
            root_path.write_bytes(canonical_json_bytes(proof_root))
            try:
                root_path.chmod(0o600)
            except OSError:
                pass
            command.extend(["--proof-root", str(root_path)])
        env = {key: value for key, value in os.environ.items() if not key.upper().startswith("AXM_")}
    ...
    try:
        ...
        fail("DIRECT_VERDICT_INVALID", "direct verifier output")
    if result.returncode != 0 or direct.get("status") != "PASS" or direct.get("bootstrapAuthenticated") is not False:
        fail("DIRECT_VERIFICATION_FAILED", str(direct.get("errorCode", "unknown")))
    if direct.get("terminal") == "PRIVATE_SELF_ATTESTED" and direct.get("privateProofAuthenticated") is not True:
        fail("PRIVATE_PROOF_AUTHENTICATION_FAILED", "direct verifier")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = sub.add_parser("validate-fixtures")
    p.add_argument("profile", type=Path)
    p.add_argument("fixtures", type=Path)
    p = sub.add_parser("evaluate-fixture")
    p.add_argument("profile", type=Path)
    p.add_argument("fixtures", type=Path)
    p.add_argument("case")
    p = sub.add_parser("build-fixture")
    p.add_argument("profile", type=Path)
    p.add_argument("fixtures", type=Path)
    p.add_argument("case")
    p.add_argument("--out", type=Path, required=True)
    p = sub.add_parser("create-proof-root")
    p.add_argument("profile", type=Path)
    p.add_argument("--campaign", required=True)
    p.add_argument("--out", type=Path, required=True)
    p = sub.add_parser("authenticate-private-input")
    p.add_argument("profile", type=Path)
    p.add_argument("fixtures", type=Path)
    p.add_argument("input", type=Path)
    p.add_argument("--proof-root", type=Path, required=True)
    p.add_argument("--out", type=Path, required=True)
    p = sub.add_parser("build-private")
    p.add_argument("profile", type=Path)
    p.add_argument("fixtures", type=Path)
    p.add_argument("input", type=Path)
    p.add_argument("--proof-root", type=Path, required=True)
    p.add_argument("--out", type=Path, required=True)
    p = sub.add_parser("verify-join")
    p.add_argument("carrier", type=Path)
    p.add_argument("--proof-root", type=Path)
    ...
    try:
        if args.command == "validate-profile":
            ...
            emit({
                ...
                "status": "PASS", "profileId": profile["profileId"],
                "profileCanonicalSha256": PROFILE_CANONICAL_SHA256,
                "privateProofRootRequired": True,
                "physicalExecutionStartedByJoin": False,
                "workersLaunched": 0, "listenersCreated": 0, "authority": "none",
            })
        elif args.command == "validate-fixtures":
            ...
            emit({
                "schema": "axm-head/physical-long-haul-fixture-validation@1",
                "status": "PASS", "cases": len(catalog["cases"]),
                "fixtureCatalogCanonicalSha256": FIXTURE_CATALOG_CANONICAL_SHA256,
                "privateSelfAttestedFixtures": 0,
                "privateProofRootsEmbedded": 0,
                "authority": "none",
            })
        elif args.command == "evaluate-fixture":
            profile = validate_exact_profile(args.profile)
            ...
            emit(build_objects(profile, row["input"])["public"])
        elif args.command == "build-fixture":
            profile = validate_exact_profile(args.profile)
            catalog = validate_exact_catalog(profile, args.fixtures)
            row = find_case(catalog, args.case)
            emit(write_carrier(profile, catalog, row["input"], args.out))
        elif args.command == "create-proof-root":
            validate_exact_profile(args.profile)
            emit(create_private_proof_root_file(args.campaign, args.out))
        elif args.command == "authenticate-private-input":
            profile = validate_exact_profile(args.profile)
            validate_exact_catalog(profile, args.fixtures)
            emit(authenticate_private_input_file(args.input, args.proof_root, args.out))
        elif args.command == "build-private":
            profile = validate_exact_profile(args.profile)
            catalog = validate_exact_catalog(profile, args.fixtures)
            proof_root = load_private_proof_root(args.proof_root)
            input_value = read_json(args.input)
            result = write_carrier(profile, catalog, input_value, args.out, proof_root)
            emit(result)
        elif args.command == "verify-join":
            proof_root = load_private_proof_root(args.proof_root) if args.proof_root is not None else None
            emit(bootstrap_verify(args.carrier, args.out, proof_root))
        return 0
