...
function fiveQuestionAnswer({ profile, bundle, replay, reconciliation }) {
  const openObligations = [
    ...
    {
      obligationId: reconciliation.obligation.obligationId,
      type: reconciliation.obligation.obligationType,
    },
  ];
  return {
    ...
    questions: {
      whatExists: {
        answer: 'One invented synthetic Mission Profile 01 state, one bounded model proposal, one denied-communications authority decision, one admission ticket, and one completed local-artifact-only task.',
        objectIds: [
      ...
      whatProvesIt: {
        answer: 'Exact package hashes plus deterministic detached replay and Lattice-removal receipts.',
        receiptIds: [
      ...
      whoMayAct: {
        answer: 'Only the bounded local authority runtime admitted the synthetic task while a local operator was present; the model, task receipt, and Lattice membrane carry no authority.',
      ...
      whatIsUnresolved: {
        answer: 'The candidate shared-corridor relationship and the external Lattice-shaped candidate both require explicit human review.',
        obligations: openObligations,
      },
      whatIsSafeNext: {
        answer: 'Inspect the cited synthetic observations and record a human disposition. Do not perform any external, kinetic, command, targeting, engagement, or effector action.',
        allowedEffect: 'local_review_only',
      },
    },
    status: 'complete_for_synthetic_qualification',
    authority: 'none',
    claimBoundary: 'This answer supports cold-successor inspection of one synthetic qualification pack. It is not field readiness, operational C2, command authority, targeting, engagement, effector, or weapons capability.',
  };
}
...
function validateSourceCommit(sourceCommit) {
  requireCondition(/^[0-9a-f]{40}$/.test(sourceCommit), 'SOURCE_COMMIT_INVALID', 'source commit must be a full lowercase Git SHA-1');
  return sourceCommit;
}
...
export async function buildColdSuccessorPack(outDir, { sourceCommit }) {
  ...
  const fiveQuestions = fiveQuestionAnswer({
    ...
    bundle,
    replay,
    reconciliation: latticeReconciliation,
  });
  ...
  const manifestBody = {
    ...
    authority: 'none',
    claimBoundary: 'This manifest binds one synthetic, source-pinned runnable qualification pack. It does not establish field readiness, target hardware qualification, production Lattice integration, operational C2, command authority, targeting, engagement, effector, or weapons capability.',
  };
  const manifest = {
    ...manifestBody,
...
function assertFiveQuestionAnswer(answer) {
  requireCondition(answer?.schema === 'spectra-anchor-node-cold-successor-five-question-answer/1', 'FIVE_QUESTION_INVALID', 'five-question answer schema is invalid');
  const keys = Object.keys(answer.questions ?? {}).sort();
  requireCondition(
    canonicalJson(keys)
      === canonicalJson(['whatExists', 'whatIsUnresolved', 'whatIsSafeNext', 'whatProvesIt', 'whoMayAct'].sort()),
    'FIVE_QUESTION_INVALID',
    'five-question answer is incomplete',
  );
  requireCondition(answer.questions.whoMayAct.modelAuthority === false, 'FIVE_QUESTION_INVALID', 'model authority is not false');
  requireCondition(answer.questions.whatIsSafeNext.allowedEffect === 'local_review_only', 'FIVE_QUESTION_INVALID', 'safe next effect is too broad');
  requireCondition(Array.isArray(answer.questions.whatIsUnresolved.obligations) && answer.questions.whatIsUnresolved.obligations.length >= 2, 'FIVE_QUESTION_INVALID', 'unresolved obligations are incomplete');
  requireCondition(answer.authority === 'none', 'FIVE_QUESTION_INVALID', 'five-question answer cannot grant authority');
}

export async function verifyColdSuccessorPack(outDir) {
  const manifest = await readJson(join(outDir, 'manifest.json'));
  requireCondition(manifest.schema === 'spectra-anchor-node-cold-successor-pack-manifest/1', 'MANIFEST_INVALID', 'manifest schema is invalid');
  validateSourceCommit(manifest.sourceCommit);
  requireCondition(manifest.classification === 'invented_unclassified_synthetic_only', 'MANIFEST_INVALID', 'manifest classification is invalid');
  requireCondition(manifest.externalServiceCalls === 0, 'MANIFEST_INVALID', 'manifest records external service calls');
  requireCondition(manifest.operationalCredentials === 0, 'MANIFEST_INVALID', 'manifest records operational credentials');
  requireCondition(manifest.latticeRequired === false, 'MANIFEST_INVALID', 'manifest makes Lattice required');
  requireCondition(manifest.authority === 'none', 'MANIFEST_INVALID', 'manifest cannot grant authority');
  requireCondition(Array.isArray(manifest.files) && manifest.files.length === PACK_FILE_NAMES.length, 'MANIFEST_INVALID', 'manifest file denominator differs');
  requireCondition(manifest.fileCount === PACK_FILE_NAMES.length, 'MANIFEST_INVALID', 'manifest file count differs');
  requireCondition(
  ...
  for (const row of manifest.files) {
    ...
    const bytes = await readFile(path);
    requireCondition(bytes.length === row.bytes, 'PACK_FILE_SIZE_MISMATCH', `pack file size differs: ${row.path}`);
    requireCondition(digestBytes(bytes) === row.sha256, 'PACK_FILE_HASH_MISMATCH', `pack file hash differs: ${row.path}`);
  }
  const manifestBody = structuredClone(manifest);
  delete manifestBody.packId;
  requireCondition(manifest.packId === digest('anchorcoldsuccessorpack1', manifestBody), 'PACK_ID_INVALID', 'pack identity is invalid');
  ...
  requireCondition(canonicalJson(storedBundle) === canonicalJson(replayedBundle), 'PACK_REPLAY_MISMATCH', 'vertical slice does not reconstruct');
  const storedReplay = await readJson(join(outDir, FILES.replay));
  requireCondition(canonicalJson(storedReplay) === canonicalJson(verifyVerticalSlice(storedBundle)), 'PACK_REPLAY_MISMATCH', 'detached replay receipt does not reconstruct');
  const storedLatticeEnvelope = await readJson(join(outDir, FILES.latticeEnvelope));
  requireCondition(canonicalJson(storedLatticeEnvelope) === canonicalJson(projectVerticalSliceToLattice(storedBundle)), 'PACK_REPLAY_MISMATCH', 'Lattice projection does not reconstruct');
  const storedLatticeRemoval = await readJson(join(outDir, FILES.latticeRemoval));
  requireCondition(canonicalJson(storedLatticeRemoval) === canonicalJson(verifyLatticeRemoval(storedBundle, storedLatticeEnvelope)), 'PACK_REPLAY_MISMATCH', 'Lattice removal receipt does not reconstruct');
  const inbound = await readJson(join(outDir, FILES.latticeInbound));
  const storedReconciliation = await readJson(join(outDir, FILES.latticeReconciliation));
  requireCondition(canonicalJson(storedReconciliation) === canonicalJson(reconcileInboundCandidate(storedBundle.missionStateAfter, inbound)), 'PACK_REPLAY_MISMATCH', 'Lattice reconciliation receipt does not reconstruct');
  ...
  const receiptBody = {
    ...
    latticeRequired: false,
    authority: 'none',
    claimBoundary: 'This receipt proves exact bytes and deterministic reconstruction of one synthetic qualification pack. It grants no operational, field, evaluator, adoption, or command authority.',
  };
  return {
    ...receiptBody,
